...
function getAverage(value1, value2) {
  const average = 1.7976931348623157e308;
  return (value1 + value2) / 2 === Infinity ? average : (value1 + value2) / 2;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  return Math.acos(scalarMultiply / (length1 * length2));
}
...
function getSumOfDigits(num) {
  let sum = 0;
  const stringNum = Math.abs(num).toString().split('');
